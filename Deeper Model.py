...
def read_batches(all_chars, unique_chars):
    ...
    for start in range(0, batch_chars - SEQ_LENGTH,
                       64):
        ...
        for batch_index in range(0, 16):
            for i in range(0, 64):
                ...
                Y[batch_index, i, all_chars[
                    batch_index * batch_chars + start + i + 1]] = 1
        yield X, Y
...
def built_model(batch_size, seq_length, unique_chars):
    ...
    model.add(
        Embedding(input_dim=unique_chars, output_dim=512, batch_input_shape=(batch_size, seq_length), name="embd_1"))
...
def training_model(data, epochs=80):
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(read_batches(all_characters, unique_chars)):
            final_epoch_loss, final_epoch_accuracy = model.train_on_batch(x,
                                                                          y)
            print("Batch: {}, Loss: {}, Accuracy: {}".format(i + 1, final_epoch_loss, final_epoch_accuracy))
            #print each learning step
